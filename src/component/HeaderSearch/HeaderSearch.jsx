...
const HeaderSearch = () => {
  ...
  return (
    ...
    //   <div className="h-40 w-full bg-red-500 max-w-[280px]"></div>
    ...
    <div className="hidden md:flex items-center gap-4">
      ...
      <div className="relative w-full max-w-[280px]">
        <button
          ...
          className="flex justify-between items-center bg-yellow-400 w-full text-white px-4 py-2 rounded-lg h-16"
        >
        ...
        {catOpen && (
          <ul className="absolute mt-2 bg-white shadow-lg rounded-lg 30 w-90 z-15 w-full">
            <li
              ...
              onMouseLeave={() => handleLeave(electronicsTimer, setElectronics)}
            >
              {" "}
              <span className={`${electronics && "text-blue-600"}`}>Electronics</span>{" "}
              <RiArrowDropUpLine
                className={`text-6xl ${
                  electronics
                    ? "text-blue-600 rotate-80 transform transition-transform duration-1000 ease-in-out"
                    : "-rotate-0 transform transition-transform duration-1000 ease-in-out"
                }`}
              />
              {electronics && (
            ...
            <li
              className="flex justify-between items-center px-6 hover:text-blue-500"
              onMouseEnter={() => handleEnter(MensFashionTimer, setMensFashion)}
              onMouseLeave={() => handleLeave(MensFashionTimer, setMensFashion)}
            >
              {" "}
              <span className={`${MensFashion && "text-blue-600"}`}>Men’s Fashion</span>{" "}
              <RiArrowDropUpLine
                className={`text-6xl ${
                  MensFashion
                    ? "text-blue-600 rotate-80 transform transition-transform duration-1000 ease-in-out"
            ...
            >
              {" "}
              <span className={`${WomensFashion && "text-blue-600"}`}>Women’s Fashion</span>{" "}
              <RiArrowDropUpLine
                className={`text-6xl ${
                  WomensFashion
                    ? "text-blue-600 rotate-80 transform transition-transform duration-1000 ease-in-out"
            ...
            <li
              ...
              onMouseLeave={() => handleLeave(OfficeSecurityTimer, setOfficeSecurity)}
            >
              <span className={`${OfficeSecurity && "text-blue-600"}`}>Office & Security </span>{" "}
              <RiArrowDropUpLine
                className={`text-6xl ${
                  OfficeSecurity
                    ? "text-blue-600 rotate-80 transform transition-transform duration-1000 ease-in-out"
      ...
      <div className="flex border border-gray-300 rounded-lg w-full">
        <input type="text" placeholder="Find your product" className="px-4 rounded-lg w-full h-16" />
    ...
    "
      ...
      {/* Promo Button */}
      <button className="relative overflow-hidden btn btn-outline btn-info whitespace-normal py-8 px-8 group transition-all duration-500 ease-in-out w-full max-w-[280px]">
        ...
        {/* Button Text */}
        <span className="relative z-10 font-bold text-info group-hover:text-white">BLACK FRIDAY</span>
        <br />
        <span className="relative z-10 text-sm text-info group-hover:text-white">Get 45% Off!</span>
      </button>
    </div>
  );
};
